...
public class RequestLogic {
	...
	public static boolean removeRequest(User user) {
		try {
			RequestPersistenceService.removeRequest(RequestPersistenceService.loadRequest(user));
	 ...
	 * 
	 * @param request The {@link Request} that should be updated.
	 */
	...
	public static RequestStatus getRequestStatus(User user) {
		return RequestPersistenceService.loadRequest(user).getRequestStatus();
	}
	 ...
	 * 
	 * @param request The {@link Request} that should be changed.
	 * @return The {@link Boolean} that describes if the process was successful.
	 */
	public static boolean postponeRequest(Request request) {
		if (request.getRequestStatus() != RequestStatus.WAITING) {
			try {
				...
				RequestPersistenceService.updateRequest(request);
				LogLogic.createLog("Antrag wurde auf 'wartend' gesetzt", request.getUser().getEmailAddress());
				return true;
			} catch (Exception e) {
				e.printStackTrace();
				LogLogic.createLog("Antrag konnte nicht auf 'wartend'gesetzt werden", request.getUser().getEmailAddress());
				return false;
			}
		}
		return true;
	...
	/**
	 * Method which returns the overall number of {@link Request}s in the database with a certain {@link RequestStatus}.
	 * 
	 * @param requestStatus
	 * @return Number of {@link Request}s with certain {@link RequestStatus}.
	 */
	public static int getNumberOfRequests(RequestStatus requestStatus) {
		return (int) RequestPersistenceService.loadAllRequests().stream().filter(request -> request.getRequestStatus() == requestStatus).count();
	}
}
